...
# file
from train_weights_edge_class import (DISCRETIZER, featurize, FILENAME)

clf = joblib.load(FILENAME)
def predict_mol(mol):
    features = featurize(mol)
    edges, feats = zip(*features.items())
    results = clf.predict(feats)
    results = results.astype(float)  # needed for the inverse_transform
    results = DISCRETIZER.inverse_transform(results.reshape(-1, 1)).ravel()
    return dict(zip(edges, results))
